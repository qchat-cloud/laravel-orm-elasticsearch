...
```php
Qchat\Elasticsearch\ElasticsearchServiceProvider::class
```

##### 3) Add package alias (< laravel 5.5).

```php
'ES' => Qchat\Elasticsearch\Facades\ES::class
```
	
##### 4) Publishing.

```bash
$ php artisan vendor:publish --provider="Qchat\Elasticsearch\ElasticsearchServiceProvider"
```
...
```php
$app->register(Qchat\Elasticsearch\ElasticsearchServiceProvider::class);
```
...
use Qchat\Elasticsearch\Connection;
...
class Post extends Model
{
     ...
     *
     * @param \Qchat\Elasticsearch\Query $query
     * @return \Qchat\Elasticsearch\Query
     */
